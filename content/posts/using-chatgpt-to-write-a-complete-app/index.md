...
slug: using-chatgpt-to-write-a-complete-app
date: 2023-05-09
tags: ["foobal", "balgpt", "chatgpt", "ai", "software-development"]
description: "In which I share my experiences building an app with ChatGPT so I can start annoying my uncle again."
aliases:
  - /post/2023/05/09/using-chatgpt-to-write-a-complete-app/
---
When I first started playing with ChatGPT, I had it write a [simple, low effort blog post](/posts/my-experience-with-chatgpt-as-a-developer-impressed-but-with-caveats/) to see what it could do (and [a follow-up using the same prompts](/posts/my-adventure-with-chatgpt-a-developers-perspective/) when GPT-4 came out). Those posts were a bit lazy. This is going to be a more serious exploration, written manually (but reviewed by ChatGPT 😉).

I wanted to write a full app with ChatGPT, just to see how far I could take it. I was still using Foobal, the soccer match predictor I use to play a pool with my relatives, which [I've](/posts/foobal-predicting-soccer-matches-with-scala-and-drools/) [also](/posts/foobal-end-of-year-update/) [blogged](/posts/foobal-a-new-hope/) [about](/posts/on-profiling/) [before](/posts/foobal-moneyball/). I never really maintained that app, while the ecosystem had progressed, so I have a hard time even building the thing these days. Also, it only has a CLI, and I want a web app. Time for an upgrade!
...
![A not-very-pretty user interface](pretty.png)
